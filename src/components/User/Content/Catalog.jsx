...
const Catalog = () => {
  ...
  const getQuizzesByMajor = async (major) => {
    try {
      ...
      console.log(`Quiz cho ${major}:`, response.data.quizzes);
      return response.data.quizzes;
    } catch (error) {
      console.error(`Lỗi khi lấy quiz cho chuyên ngành ${major}:`, error.response?.data || error.message);
      return [];
    }
  };
  ...
  useEffect(() => {
    const fetchAllQuizzes = async () => {
      ...
      try {
        ...
        // Log toàn bộ quizzesByMajor để gỡ lỗi
        console.log('Danh sách quiz theo chuyên ngành:', quizzesMap);
        ...
        console.log('Tất cả quiz:', allQuizzes);
        ...
        console.log('Quiz yêu thích:', favoriteQuizzes);
        console.log('Quiz đề xuất:', suggestedQuizzes);
        setFavoriteQuizzes(favoriteQuizzes);
        setSuggestedQuizzes(suggestedQuizzes);
      } catch (err) {
        setError('Không thể tải dữ liệu quiz: ' + (err.message || 'Lỗi không xác định'));
        console.error('Lỗi khi lấy tất cả quiz:', err);
      } finally {
        setIsLoading(false);
      }
    };
